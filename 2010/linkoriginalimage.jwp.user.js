...
var ImgLinks = {
	...
	getLookupFunc: function(startImgId, img) {
		return function resultParser(res) {
			try {
				txt = res.responseText;
				//jwp: 20100207 - original Flickr images can have the more than jus the jpg extension: [jpg|png|gif]
				var re = new RegExp('<img src="([^"]+' + startImgId + '.+\.[jpg|png|gif])"');
				...
				if (m) {
					...
					if (txt.indexOf("Flickr: Private page") > -1) {
						...
						myLog('No image found (unknown): "' + startImgId + '"');
                        myLog(txt);
					}
					return;
				}
